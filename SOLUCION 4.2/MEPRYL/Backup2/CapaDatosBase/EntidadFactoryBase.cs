...
namespace CapaDatosBase
{
    public abstract class EntidadFactoryBase
    {
        ...
        //Viviana
        public DataTable obtenerClubxLiga(string palabrasClave)
        {
            DataTable tabla = null;
            try
            {
                ...
                for (int i = 0; i < palabras.Length; i++)
                {
                    if (palabras[i].Trim() != "")
                    {
                        filtro += " AND l.id LIKE '%" + palabras[i].Trim() + "%' ";
                    }
                }

                SqlParameter[] param = new SqlParameter[1];
                param[0] = new SqlParameter("@filtro", filtro);
                DataSet ds = SqlHelper.ExecuteDataset(configuracion.getConectionString(), "sp_" + this.EntidadNombre + "_getByCodigoLiga", param);
                if (ds.Tables.Count > 0)
                    tabla = ds.Tables[0];
        ...
        //Viviana
        public DataTable getSelectBusClub(string palabrasClave,string liga)
        {
            DataTable tabla = null;
            try
            {
                ...
                for (int i = 0; i < palabras.Length; i++)
                {
                    if (palabras[i].Trim() != "")
                    {
                        filtro += " AND descripcion LIKE '%" + palabras[i].Trim() + "%' and ligaId = '" + liga + "'";
                    }
                }

                SqlParameter[] param = new SqlParameter[1];
                param[0] = new SqlParameter("@filtro", filtro);
                DataSet ds = SqlHelper.ExecuteDataset(configuracion.getConectionString(), "sp_" + this.EntidadNombre + "_SelectFiltroBusClub", param);
                if (ds.Tables.Count > 0)
                    tabla = ds.Tables[0];
        ...
        public string borrar(Guid id)
        {
            ...
            try
            {
                ...
                {
                    SqlHelper.ExecuteNonQuery(this.configuracion.getConectionString(), "sp_" + this.EntidadNombre + "_DeleteClub", param);
                }
