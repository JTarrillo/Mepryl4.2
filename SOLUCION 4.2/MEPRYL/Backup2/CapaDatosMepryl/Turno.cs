...
namespace CapaDatosMepryl
{
    public class Turno
    {
        ...
        public DataTable cargarTurnos(Guid tipoExamen, DateTime fecha, string hora, string estado)
        {
            ...
            if (tipoExamen != Guid.Empty) { filtroTipoExamen = " and tep.id = '" + tipoExamen.ToString() + "'"; }
        ...
        private DataTable generarTablaRetornoTurno(DataTable turnos)
        {
            try
            {
                ...
                foreach (DataRow r in turnos.Rows)
                {
                    ...
                    if (r.ItemArray[6].ToString() != Guid.Empty.ToString())
                    {
                    ...
                    {
                        estado = "1";
                        if (Convert.ToBoolean(r.ItemArray[10].ToString())) { estado = "3"; }
                        if (r.ItemArray[12].ToString() == "1") { estado = "4"; }
                        if (r.ItemArray[14].ToString() == "0") { estado = "5"; }
        ...
        public DataTable cargarTablaInformacionTurno(Guid idTurno)
        {
            ...
            t.pacienteID, t.observaciones, tep.id, e.descripcion, tep.modificado, 
            tep.factClub, tep.precioExamen
            from dbo.Turno t 
        ...
        public Entidades.TurnoLaboral cargarTurnoPacienteLaboral(Guid idTurno)
        {
            ...
            if (infoTurno.Rows.Count > 0)
            {
                ...
                retorno.TipoExamen = tipoExamen.cargarEstudiosPorExamen(infoTurno.Rows[0][3].ToString());
                DataTable infoPaciente = SQLConnector.obtenerTablaSegunConsultaString(@"select id, 
                apellido + ' ' + nombres, telefonos, celular, dni, cuil from dbo.PacienteLaboral
                where id = '" + infoTurno.Rows[0][1].ToString() + "'");
                if (infoPaciente.Rows.Count > 0)
                {
        ...
        private void updateReserva(Guid idTurno, string estado, object destinatario)
        {
            ...
            if (estado == "1") { idUsuario = new Guid(Configuracion.usuario.id); }
            List<string> updateUsuario = SQLConnector.generarListaParaProcedure("@idTurno", "@idUsuario");
        ...
        public DataTable cargarTurnosConFiltro(string filtro)
        {
            ...
            strSQL = "select t.id as Id, tep.descripcion as TipoExamen, " +
				     ...
				     "WHERE PC.dni LIKE '%" + filtro + "%' OR t.codigo LIKE '%" + filtro + "%' OR " +
					       "PC.apellido LIKE '%"+ filtro +"%' OR PC.nombres = '%"+ filtro +"%' " +
                     "order by t.fecha, t.hora";
...
//                    order by t.fecha, t.hora");
        ...
        public DataTable PacienteTieneTurnoAsignado(DateTime fecha, string idPaciente)
        {
            //bool blnTieneTurno = false;
            string strSQL = "";
            ...
            strSQL = "select t.id as Id, tep.descripcion as TipoExamen, " +
                     ...
                     "inner join dbo.Horario h on t.horarioID = h.id inner join dbo.Profesional p on h.profesionalID = p.id " +
                     "inner join dbo.Especialidad tep on h.especialidadID = tep.id " +
                     "where (convert(date,t.fecha) > convert(date,GETDATE(),105) AND convert(date,t.fecha) < convert(date,'01/01/2086',105)) and t.pacienteID = '" + idPaciente + "' order by t.fecha, t.hora";            
        ...
        {            
            ...
            strSQL = "SELECT TOP 1 fecha, PacienteID, UsuarioID FROM dbo.Turno WHERE PacienteID <> '00000000-0000-0000-0000-000000000000' AND id = '" + strIdTurno + "'";
