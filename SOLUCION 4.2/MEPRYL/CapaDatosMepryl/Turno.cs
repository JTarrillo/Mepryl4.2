...
namespace CapaDatosMepryl
{
    public class Turno
    {
        ...
        private void actualizarPrecioTipoExamenPorPeriodo(Entidades.TipoExamen entidadTipoExamen, Guid idEspecialidad, DateTime fechaTurno)
        {
            ...
            DataRow row = precioPublico.Rows[0];
            if (row["PrecioPromo"] != DBNull.Value)
        ...
        public Entidades.TurnoPreventiva nuevoTurnoPacientePreventiva(string idPaciente, string idTurno)
        {
            ...
            if (infoTurno.Rows.Count > 0)
            {
                ...
                retorno.TipoExamen = tipoExamen.cargarEstudiosPorTipoExamen(retorno.IdTipoExamen.ToString());
                actualizarPrecioTipoExamenPorPeriodo(retorno.TipoExamen, retorno.IdTipoExamen,
                    Convert.ToDateTime(infoTurno.Rows[0][3].ToString()));
                if (blnInfantilInicial == false)
                {
        ...
        public Entidades.Resultado asignarTurnoPacientePreventivaVentanillaMesaEntrada(string idPaciente, string idTurno)
        {
            ...
            if (ppVent.Rows.Count > 0)
            {
                ...
                entidad.TipoExamen.PrecioLista = Convert.ToDouble(ppVent.Rows[0]["PrecioLista"].ToString());
            }
        ...
        public DataTable ObtenerPrecioPromo(Guid idEspecialidad, DateTime fecha)
        {
            DataTable dt = SQLConnector.obtenerTablaSegunConsultaString(
                ...
                "FROM dbo.PrecioPromo pp " +
                "LEFT JOIN dbo.PrecioPublico pu ON pp.idEspecialidad = pu.idEspecialidad AND pp.Mes = pu.Mes AND pp.Anio = pu.Anio AND pu.Eliminado = 0 " +
