    ...
    public class Empresa
    {
        ...
        public Entidades.Resultado alta(Entidades.Empresa e)
        {
            ...
            { 
                ...
                {
                    str = pi.Name.ToString();
                    if (str == "Art")
                    {
                        str = e.GetType().GetProperty(pi.Name).GetValue(e, null).ToString();
                        str = e.GetType().GetProperty(pi.Name).GetValue(e, null).ToString();
                    }
                    str = e.GetType().GetProperty(pi.Name).GetValue(e, null).ToString();
                    str = e.GetType().GetProperty(pi.Name).GetValue(e, null).ToString();
        ...
        {         
            ...
            if (empresa.Rows.Count > 0)
            {
                ...
                if (empresa.Rows[0][33].ToString() != string.Empty) { retorno.ListaPrecios = int.Parse(empresa.Rows[0][33].ToString()); }
                ...
                retorno.EmpresaPrincipal = cargarDatoBool(empresa.Rows[0][49]);
            }
            return retorno;     
        }
        ...
        private DataTable consultaDeEmpresas(string filtro)
        {
            ...
            localidad as 'Localidad', provincia as 'Provincia', tipoDeEntidad as 'Tipo'
            from dbo.Empresa " + filtro + " order by razonSocial");
                try
                {
                    foreach (DataRow r in consulta.Rows)
                    {
                        ...
                        retorno.Rows.Add(r.ItemArray[0], r.ItemArray[1], r.ItemArray[2],
                            ...
                            r.ItemArray[5], r.ItemArray[6], r.ItemArray[7], r.ItemArray[8], r.ItemArray[9]);
                    }
        ...
        private void EstablecerEmpresaPrincipal(string strIdEmpresa)
        {
            string strSQL = "";
            strSQL = "update dbo.Empresa " +
                     "set Principal = 1 " +
        ...
        {
            string strSQL = "";
            strSQL = "SELECT id, Descripcion, Valor FROM dbo.IVA WHERE NOT Descripcion = 'PREDETERMINADO' order by descripcion";
        ...
        {
            string strSQL = "";
            strSQL = "INSERT INTO dbo.Iva " +
                     "(id, descripcion, valor) " +
                     "VALUES " +
                     "(NEWID(), '" + strNombre + "', '" + strValor + "')";
