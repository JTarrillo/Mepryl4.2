...
namespace CapaDatosMepryl
{
    public class PrecioPublico
    {
        ...
        {
            string strSQL = "SELECT Coeficiente FROM CoeficientePrecio WHERE Mes = " + mes + " AND Anio = " + anio + " AND Tipo = 'PUBLICO'";
            DataTable dt = SQLConnector.obtenerTablaSegunConsultaString(strSQL);
        ...
        public void GuardarCoeficiente(int mes, int anio, decimal coeficiente)
        {
            ...
            string strSQL = "IF EXISTS (SELECT 1 FROM CoeficientePrecio WHERE Mes = " + mes + " AND Anio = " + anio + " AND Tipo = 'PUBLICO') " +
                            "UPDATE CoeficientePrecio SET Coeficiente = " + coef + ", FechaModificacion = GETDATE() " +
                            "WHERE Mes = " + mes + " AND Anio = " + anio + " AND Tipo = 'PUBLICO' " +
                            "ELSE " +
                            "INSERT INTO CoeficientePrecio (Mes, Anio, Coeficiente, Tipo) VALUES(" + mes + ", " + anio + ", " + coef + ", 'PUBLICO')";
            SQLConnector.obtenerTablaSegunConsultaString(strSQL);
        }
        ...
        {
            string strSQL = "SELECT Mes, ISNULL(Coeficiente, 1) AS Coeficiente FROM CoeficientePrecio WHERE Anio = " + anio + " AND Tipo = 'PUBLICO' ORDER BY Mes";
            return SQLConnector.obtenerTablaSegunConsultaString(strSQL);
        }
        ...
        public void GuardarCoeficientesAnio(int anio, decimal[] coeficientes)
        {
            try
            {
                ...
                for (int mes = 1; mes <= 12; mes++)
                {
                    ...
                    sb.Append("IF EXISTS (SELECT 1 FROM CoeficientePrecio WHERE Mes = " + mes + " AND Anio = " + anio + " AND Tipo = 'PUBLICO') ");
                    sb.Append("UPDATE CoeficientePrecio SET Coeficiente = " + coef + ", FechaModificacion = GETDATE() WHERE Mes = " + mes + " AND Anio = " + anio + " AND Tipo = 'PUBLICO' ");
                    sb.AppendLine("ELSE INSERT INTO CoeficientePrecio (Mes, Anio, Coeficiente, Tipo) VALUES(" + mes + ", " + anio + ", " + coef + ", 'PUBLICO');");
                }
                SQLConnector.obtenerTablaSegunConsultaString(sb.ToString());
            }
            catch (Exception ex)
