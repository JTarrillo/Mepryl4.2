...
namespace CapaDatosMepryl
{
    public class PacientePreventiva
    {
        ...
        public DataTable ListarLigaCategoriaDescripcion(int intCategoria, string strDescripcion)
        {
            ...
            strSQL = "SELECT L.id, L.Codigo, L.descripcion FROM " +
                     "dbo.ParametrosPlacas P INNER JOIN dbo.Liga L ON P.ligasIDs = CONVERT(VARCHAR(36), L.id) AND L.activo = 1 " +
                     "WHERE (((P.categoriaInicial >= " + intCategoria + ") AND (" + intCategoria + " >= P.novenaCategoria)) OR (AdmiteMenores = 1 AND (" + intCategoria + " >= P.categoriaInicial)) OR L.codigo = 0) AND descripcion LIKE '%" + strDescripcion + "%' ORDER BY L.descripcion ASC";
            return SQLConnector.obtenerTablaSegunConsultaString(strSQL);
        }
        ...
        {   
            ...
            try
            {
                ...
                strIdPaciente = ObtenerIDpaciente(strDNI);

                if (!string.IsNullOrEmpty(strIdPaciente))
                    if (ObtenerIDligaPaciente(ObtenerIDclubPaciente(strIdPaciente)).ToUpper() == "345FFF9B-45C2-4CD5-87EC-47E944E8236D")
                        blnAFAInfantil = true;
            }
                    ...
                    if (!string.IsNullOrEmpty(dtTabla.Rows[i].ItemArray[1].ToString()))
                    {
                        ...
                        {                            
                            ...
                            if (!VerificaRX(ObtenerIDligaPaciente(ObtenerIDclubPaciente(ObtenerIDpaciente(strDNI)))))
                                blnResultado = true;
                            if (VerificaTieneExamenAFAInfantil(strDNI) == false && blnAFAInfantil == true)
                                blnResultado = true; blnAFAInfantil = false;
                        }
                    }
                }
            }
        ...
        public bool VerificaTieneExamenAFAInfantil(string strDNI)
        {
            ...
            strSQL = @"select tep.id as IdTE, c.id as IdC, 
                    CONVERT(date, c.fecha) as Fecha, c.identificador as 'Nº Examen', p.dni as DNI,
                    (p.apellido + ' ' + p.nombres) as Paciente, tep.rm as RM, tep.imp as IMP, tep.inf as INF, tep.mail as Mail, tep.dictAut, tep.ImpLab,
                    p.fechaNacimiento, tep.cons 
                    FROM dbo.Consulta c 
                    ...
                    INNER JOIN dbo.Club CLU ON CPE.idClub = CLU.id 
                    INNER JOIN dbo.Liga LIG ON LIG.id = CLU.ligaID
                    where LIG.id = '345FFF9B-45C2-4CD5-87EC-47E944E8236D' AND c.tipo = 'P' and YEAR(Convert(date,c.fecha)) >= (YEAR(GETDATE()) - 1)
                    AND p.dni = '" + strDNI + @"'";
        ...
        public DataTable ListarPacientePorDNI(string strDNI)
        {
            ...
            strSQL = "SELECT top 1 * FROM dbo.Paciente WHERE dni = '" + strDNI + "'";
