    ...
    public class PacienteLaboral
    {
        ...
        public Entidades.PacienteLaboral leerDatosEntidad(string idPaciente, string idEmpresa)
        {
            ...
            if (tabla.Rows.Count > 0)
            {
                ...
                paciente.EsProfesional = Convert.ToBoolean(tabla.Rows[0].ItemArray[15].ToString());
        ...
        private DataTable cargarHistoriaClinica(string idPaciente,string idEmpresa)
        {
            ...
            return historiaClinica.cargarHistoriaClinicaLaboralSinEmpresa(idPaciente, idEmpresa);
        }
        ...
        public Entidades.Resultado guardarDatosEntidadEdicion(Entidades.PacienteLaboral paciente)
        {
            ...
            try
            {
                ...
                "@localidad","@nacionalidad","@sexo","@entreCalle1","@entreCalle2","@cuil","@mail", "@profesional");
                ...
                Utilidades.guardarString(paciente.Mail), convertirBit(paciente.EsProfesional));
        ...
        public DataTable BuscarPacienteLaboral(string Clave)
        {
            ...
            strSQL = "select TOP 300 p.id, emp.id, (p.apellido + ' ' + p.nombres) as Paciente, p.dni as DNI, emp.razonSocial as Empresa, Convert(varchar(10),CONVERT(date,p.fechaNacimiento,106),103) as Nacimiento, " +
                     "epp.tarea as Tarea " +
        ...
        public DataTable MailPacientePorEmpresa(string strIdPacienteLaboral, string strIdEmpresa)
        {
            DataTable dt = null;
            string strSQL = "";

            //strSQL = "SELECT PL.Nombres, PL.Apellido, E.mail1 AS 'Email', E.razonSocial, PL.mail as 'EmailPaciente' from dbo.PacienteLaboral PL inner join dbo.EmpresasPorPaciente EPP on EPP.idPaciente = PL.id " +
            //         "inner join dbo.Empresa E on E.id = EPP.idEmpresa " +
            //         "WHERE PL.dni = '" + strDNI + "'";
            strSQL = "SELECT TOP 1 PL.Nombres, PL.Apellido, E.mail1 AS 'Email', E.razonSocial, PL.mail as 'EmailPaciente', EPP.ingreso " +
                            "from dbo.PacienteLaboral PL " +
                            "inner join dbo.EmpresasPorPaciente EPP on EPP.idPaciente = PL.id " +
                            "inner join dbo.Empresa E on E.id = EPP.idEmpresa WHERE E.Id = '" + strIdEmpresa + "' " +
                            "AND PL.id = '" + strIdPacienteLaboral + "'";
        ...
        public DataTable ListaProfesionalLabora(string strDNI)
        {
            DataTable dt = null;
            string strSQL = "";

            strSQL = "SELECT PRO.id as 'IdProfesional', PAL.id as 'IdPaciente', EMP.id as 'idEmpresa', " +
                     "EMP.razonSocial, PAL.apellido, PAL.nombres, PAL.dni, PAL.cuil, PAL.fechaNacimiento, PAL.telefonos, " +
                     "PAL.celular, PAL.direccion, PAL.localidad, PAL.nacionalidad, EPP.tarea, PAL.mail, PRO.codigoPostal, " +
                     "PRO.lugarMatricula, PRO.nroMatricula, PRO.especialidad, PRO.visitasCapital, PRO.visitasProvincia, PRO.Firma, PRO.Activo, NroDocumento " +
                     "FROM dbo.Profesional PRO " +
                     "INNER JOIN PacienteLaboral PAL ON PRO.DNI = PAL.dni " +
                     "INNER JOIN EmpresasPorPaciente EPP ON EPP.idPaciente = PAL.id " +
                     "INNER JOIN Empresa EMP ON EMP.id = EPP.idEmpresa " +
                     "WHERE PRO.dni = '" + strDNI + "'";
        ...
        public string idProfesional(string strDNI)
        {
            string strResultado = "";
            string strSQL = "";
            DataTable dt = null;

            strSQL = "SELECT id FROM dbo.Profesional where dni = '" + strDNI + "'";
            dt = SQLConnector.obtenerTablaSegunConsultaString(strSQL);

            if(dt.Rows.Count > 0)
            {
                strResultado = dt.Rows[0].ItemArray[0].ToString(); 
            }

            return strResultado;
        }

        public DataTable ListaProfesional(string strDNI)
        {            
            string strSQL = "";
        ...
        public bool EsEmpresaPrincipal(string strIdEmpresa)
        {
            DataTable dt = null;
            string strSQL = "";
            bool blnRetorno = false;

            strSQL = "SELECT principal FROM dbo.Empresa " +
                     "WHERE id = '" + strIdEmpresa + "'";

            dt = SQLConnector.obtenerTablaSegunConsultaString(strSQL);

            if (dt.Rows.Count > 0)
            {
                blnRetorno = Convert.ToBoolean(dt.Rows[0][0].ToString());
            }
