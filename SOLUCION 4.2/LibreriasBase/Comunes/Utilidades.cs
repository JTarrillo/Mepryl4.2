...
namespace Comunes
{
	...
	public class Utilidades
	{
        ...
        public static string comboObtenerID(ref ComboBox cb)
        {
            ...
            if (dt != null)
                if (dt.Rows.Count > 0 && cb.SelectedIndex > -1)
                {
                    ...
                    string col = dt.Columns.Contains("ID") ? "ID" : (dt.Columns.Contains("id") ? "id" : null);
                    if (col != null)
                        resultado = dt.Rows[cb.SelectedIndex][col].ToString();
                }
            dt = null;
            return resultado;
        }
        ...
        public static bool validarCampos(DataTable campos, string subDictamen)
        {
            DataTable codigoDictamen = SQLConnector.obtenerTablaSegunConsultaString("select v.codigoInterno from dbo.Validaciones v where v.id = " + subDictamen);
            string codigoSubDictamen = codigoDictamen.Rows[0].ItemArray[0].ToString();
            DataTable codigos = new DataTable();
            codigos.Columns.Add("Valor");
            foreach (DataRow row in campos.Rows)
            {
                DataTable temp = SQLConnector.obtenerTablaSegunConsultaString(@"select c.codigo from dbo.Validaciones v inner join
                dbo.Clasificaciones c on v.idClasif = c.id where v.id = " + row.ItemArray[0].ToString());
                if (temp.Rows.Count > 0)
                {
                    codigos.Rows.Add(temp.Rows[0].ItemArray[0]);
                }
            }
            bool retorno = false;
            if (Convert.ToInt16(codigoSubDictamen) == 4)
            {
                retorno = comprobarSiExistenValores("3", codigos);
            }

            if (Convert.ToInt16(codigoSubDictamen) == 2)
            {
                if (!comprobarSiExistenValores("3", codigos))
                {
                    retorno = comprobarSiExistenValores("2", codigos);
                }
            }

            if ( Convert.ToInt16(codigoSubDictamen) == 1)
            {
                retorno = comprobarValoresDictamenApto("1", codigos);
            }

            if (Convert.ToInt16(codigoSubDictamen) == 3 || Convert.ToInt16(codigoSubDictamen) == 5 || Convert.ToInt16(codigoSubDictamen) == 6)
            {
                retorno = true;
            }
            return retorno;
        ...
        public static DataTable crearTabla(params object[] valores)
        {
            DataTable tabla = new DataTable();
            tabla.Columns.Add("IdVal");
        ...
        private static bool comprobarValoresDictamenApto(string valor, DataTable tb)
        {
            foreach (DataRow r in tb.Rows)
            {
                if (r.ItemArray[0].ToString() != valor) { return false; }
            }
            return true;
        }

        public static string obtenerIdRango(string codigo, string valor)
        {
             DataTable validacRango = SQLConnector.obtenerTablaSegunConsultaString(@"select v.id, v.rangoDesde, v.rangoHasta
                 from dbo.Validaciones v where v.codigo = " + codigo);
                foreach (DataRow row in validacRango.Rows)
                {
                    if (Convert.ToDouble(valor) >= Convert.ToDouble(row.ItemArray[1].ToString())
                        && Convert.ToDouble(valor) <= Convert.ToDouble(row.ItemArray[2].ToString()))
                    {
        ...
        private static bool comprobarSiExistenValores(string valor, DataTable tb)
        {
            foreach (DataRow r in tb.Rows)
            {
                if (r.ItemArray[0].ToString() == valor) { return true; }
            }
            return false;
        }

        public static string calcularCuil(string dni, string sexo)
        {
            int resultado;
            int prefijo = 0;
            int sufijo = 0;
            ...
            try
            {
                ...
                if (dni != string.Empty && sexo != string.Empty)
                {

                    resultado = ((documento[0] - '0') * 3)
                    + ((documento[1] - '0') * 2)
                    + ((documento[2] - '0') * 7)
                    + ((documento[3] - '0') * 6)
                    + ((documento[4] - '0') * 5)
                    ...
                    return prefijo.ToString() + "-" + dni.ToString() + "-" + sufijo.ToString();
                }
                return string.Empty;
            }
            catch
        ...
        public static object guardarString(string cadena)
        {
        ...
        public static object guardarGuid(Guid identificador)
        {
        ...
        public static object guardarFecha(DateTime fecha)
        {
            if (fecha != new DateTime(1800, 1, 1))
            {
                return fecha;
            }
            return null;
        ...
        public static object guardarInt(int valor)
        {
            if (valor != -1)
            {
                return valor;
        ...
        public static void AbrePopUps(Form frm)
        {
            frm.StartPosition = FormStartPosition.Manual;
            frm.Location = new System.Drawing.Point(46, 72);
            frm.Show();
        }
	}
}