...
namespace Comunes
{
	...
	public class UsuarioFactory
	{
        ...
        public string getVendedorID(string nombreUsuario)
        {
            try
            {
                ...
                param[0] = new SqlParameter("@usuarioID", usuario.id);
                SqlDataReader dr = SqlHelper.ExecuteReader(this.strConexion, "sp_getVendedorByUsuarioID", param);
        ...
        public bool UsuarioActivo(string strNombreUsuario)
        {
            bool blnResultado = false;
            string strSQL = "";
            DataTable dt = null;

            strSQL = "SELECT * from dbo.Usuario WHERE username = '" + strNombreUsuario + "' AND Activo = 1";
        ...
        public string DevuelveFoto(string strNombreFoto)
        {
            string strPath = "";
            string strSQL = "";
            DataTable dt = null;
            
            strSQL = "SELECT TOP 1 UbicacionLaboral FROM dbo.UbicacionFotos WHERE IdUbicacion = 1";
            dt = SQLConnector.obtenerTablaSegunConsultaString(strSQL);

            if (dt.Rows.Count > 0)
            {
                strPath = dt.Rows[0][0].ToString();
            }

            strSQL = "SELECT PAL.dni FROM dbo.Usuario USU " +
                     "INNER JOIN PacienteLaboral PAL ON PAL.dni = USU.dni " +
                     "WHERE USU.username = '" + strNombreFoto + "'";
            dt = SQLConnector.obtenerTablaSegunConsultaString(strSQL);

            if (dt.Rows.Count > 0)
            {
                strPath = strPath + "\\" + dt.Rows[0][0].ToString() + ".jpg";
            }
