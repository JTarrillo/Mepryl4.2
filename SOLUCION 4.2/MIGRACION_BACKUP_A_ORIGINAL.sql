-- ============================================================================
-- SCRIPT DE MIGRACIÓN: Backup Mejorado → Tabla Original
-- Propósito: Actualizar la tabla TipoExamen con datos corregidos del backup
-- Fecha: 2025-11-26
-- ============================================================================

-- PASO 1: Verificar la integridad de datos antes de migrar
-- ============================================================================
PRINT '=== PASO 1: Verificando integridad de datos originales ==='
...
-- Mostrar referencias rotas (Padre que no existen)
SELECT DISTINCT
    IdTipoExamen,
    Numero,
    Nombre,
...
-- ============================================================================
-- PASO 2: ACTUALIZACIÓN DE REGISTROS - Cambiar valores de EsPadre
-- ============================================================================
PRINT '=== PASO 2: Actualizando referencias de EsPadre a 0 (subtipos) ==='

-- Los siguientes son SUBTIPOS, no PADRES (cambiar EsPadre de 1 a 0)
UPDATE TipoExamen SET EsPadre = 0 WHERE IdTipoExamen = '5A89F848-B153-4D78-B6A6-00F1A5E89E40'  -- REPETICIONES (subtipo)
...
UPDATE TipoExamen SET EsPadre = 0 WHERE IdTipoExamen = '6FBED54E-7EFA-4162-A006-92FDE7DA9071'  -- CONSULTA CON ESPECIALISTA (duplicado)
UPDATE TipoExamen SET EsPadre = 0 WHERE IdTipoExamen = '48AD474E-FF97-4345-8BED-93219CD06D68'  -- FUTBOL AFA
UPDATE TipoExamen SET EsPadre = 0 WHERE IdTipoExamen = '19160B7C-6B3B-47E2-8FD0-94EB26129953'  -- TRAUMATOLOGIA (subtipo)
...
-- ============================================================================
-- PASO 3: ACTUALIZAR REFERENCIAS DE PADRE (IdPadre)
-- ============================================================================
PRINT '=== PASO 3: Actualizando referencias de IdPadre ==='

-- Actualizar IdPadre con valores correctos del backup
...
-- ============================================================================
PRINT '=== PASO 4: Verificando migración completada ==='

-- Contar registros actualizados
SELECT 
    COUNT(*) as 'Total Registros',
    SUM(CASE WHEN EsPadre = 1 THEN 1 ELSE 0 END) as 'Padres (EsPadre=1)',
    SUM(CASE WHEN EsPadre = 0 THEN 1 ELSE 0 END) as 'Subtipos (EsPadre=0)'
FROM TipoExamen

-- Mostrar referencias rotas después de migración (debe estar vacío o mínimo)
PRINT ''
...
-- ============================================================================
-- PASO 5: ROLLBACK OPCIONAL (comentado)
-- ============================================================================
-- Si necesitas deshacer los cambios, descomenta las siguientes líneas:
--
-- BEGIN TRANSACTION
-- ... (ejecutar el script)
...
PRINT 'Los datos han sido actualizados según el backup mejorado.'
PRINT 'Verifica los resultados antes de hacer cambios definitivos.'
